...
func (user *User) doMessage(msg string) {
	...
	if msg == "users" {
	...
	} else if regExp := regexp.MustCompile(`^to:[^:]+:[^:]+$`); regExp.Match([]byte(msg)) {
		// 格式 to:xxx:abc
		// 1.获取对方的用户名
		remoteName := strings.Split(msg, ":")[1]
		if remoteName == "" {
			user.sendMsg2Client("The format is incorrect,please use「to:xxx:abc」")
			return
		}

		// 2.根据用户名 得到对方User对象
		remoteUser, ok := user.server.OnlineUserMap[remoteName]
		if !ok {
			user.sendMsg2Client("The user does not exist")
			return
		}
		...
		if content == "" {
			user.sendMsg2Client("no content")
			return
		}
		remoteUser.sendMsg2Client(user.Name + " chat with you: " + content)
	} else {
